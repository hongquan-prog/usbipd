...
function toggleLang() {
  ...
  var isZh = btn.textContent.includes('EN');
  zh.forEach(function(el) {
...
<button id="langBtn" onclick="toggleLang()">显示中文</button>
...
## Architecture<span class="zh"> / 架构设计</span>
...
1. **Transport Layer** - <span class="zh">抽象网络传输接口，支持 TCP/串口/自定义</span>
2. **Protocol Layer** - <span class="zh">USBIP 协议编解码，与传输层解耦</span>
3. **Device Driver Layer** - <span class="zh">设备驱动抽象接口，实现自定义 USB 设备</span>

---

## <span class="zh">虚拟设备</span> / Virtual Devices
...
## <span class="zh">目录结构</span> / Directory Structure
...
### <span class="zh">GPIO 配置</span> / GPIO Configuration
...
### <span class="zh">日志等级</span> / Log Levels

| Value | Level | Description |
|-------|-------|-------------|
| 0 | LOG_LEVEL_NONE | No output / 无输出 |
| 1 | LOG_LEVEL_ERR | Errors only / 错误 |
| 2 | LOG_LEVEL_WRN | Warnings / 警告 |
| 3 | LOG_LEVEL_INF | Info (default) / 信息 (默认) |
| 4 | LOG_LEVEL_DBG | Debug / 调试 |

### <span class="zh">配置步骤</span> / Configuration Steps
...
### <span class="zh">重置到默认值</span> / Reset to Defaults
...
## <span class="zh">调试器测试</span> / Debugger Testing
...
### OpenOCD Flash (<span class="zh">推荐</span> / Recommended)
...
## <span class="zh">添加自定义设备</span> / Add Custom Device
...
## <span class="zh">协议支持</span> / Protocol Support
...
## <span class="zh">已知问题</span> / Known Issues

<span class="zh">无</span> / (None)

---

## <span class="zh">许可证</span> / License
