...
#define CPU_CLOCK               2400 * 1000000U      ///< Specifies the CPU Clock in Hz.

/// Number of processor cycles for I/O Port write operations.
#define IO_PORT_WRITE_CYCLES    10U

/// SWD Mode available
#define DAP_SWD                 1               ///< SWD Mode:  1 = available
...
__STATIC_INLINE uint8_t DAP_GetVendorString (char *str) {
    uint8_t len;
    const char *Vendor = "Raspberry Pi";
    strcpy(str, Vendor);
...
__STATIC_INLINE uint8_t DAP_GetProductString (char *str) {
    uint8_t len;
    const char *Product = "RPi CMSIS-DAP";
    strcpy(str, Product);
...
__STATIC_INLINE uint32_t TIMESTAMP_GET (void) {
    ...
    return (uint32_t)(ts.tv_sec * 1000000LL + ts.tv_nsec / 1000LL);
}
